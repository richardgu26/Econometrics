...
1. for plain Octave, these allegations are true, in some respects at least
2. Octave is free, and it runs on all popular operating systems
plain Octave is perfectly good for ordinary econometrics of the sort that most students and many researchers need to do
3. Octave can easily be linked to parallel matrix algebra libraries so that it uses all cores of a computer. Plain Octave doesn't do that by default.
4.  Octave can be compiled using proprietary compilers, if you have them.
5. it is easy to use C++ code for bottlenecks. For serious research work on computationally demanding problems, this step is fundamental
6. it is easy to use OpenMP or MPI to use Octave for parallel computing. This can have a great performance effect, and your code is free and portable.
7. if you use these methods, you can get very good performance with Octave.
