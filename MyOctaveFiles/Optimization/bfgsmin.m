...
function [parameter, obj, convergence, iters] = bfgsmin(f, f_args, control)
	...
	if (length(control) == 5) control{6} = 1e-8; endif # tolerance for function convergence
	if (length(control) == 6) control{7} = 1e-4; endif # tolerance for parameter convergence
	if (length(control) == 7) control{8} = 1e-4; endif # tolerance for gradient convergence
